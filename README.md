...
| **统计** | 字数、各板块数量、API 费用 |
| **背诵版** | 读懂后可一键生成背诵材料：意群分块 + 提示卡 + 遮罩自测 + 艾宾浩斯复习计划 + 每日打卡 |

## 读懂 → 背下（背诵版）

解读看板内自带「背诵」Tab（在「金句单词」后面）：对同一材料补跑一次 AI 意群分块，
直接在看板里查看可交互的背诵材料（原文分块、关键词/首字提示卡、遮罩自测、
第 1/2/4/7/15 天逐块复习表、每日打卡清单），打卡进度保存在本机浏览器。

- 首次打开「背诵」Tab 点「🚀 生成背诵版」，完成后自动回到看板并展开背诵内容
- 「🖨 打印 / 独立打开背诵版」可查看独立背诵页 `/recite/result/<id>`（其内部打卡与看板内打卡互通）
- 文库中带「📖 已生成背诵版」角标的材料均已生成背诵版
- 原文保存在 `output/<id>/source.json`，供背诵版复用（智读分析本身不受影响）
